...
ESX.RegisterCommand(
  ...
  function(xPlayer, args, showError)
    ...
    Core.SavePlayer(targetPlayer, function() end)
...
ESX.RegisterCommand(
  ...
  "admin",
  function(xPlayer, args, showError)
    if not ESX.DoesOrgaExist(args.orga, args.grade) then
      return showError(TranslateCap("command_setorga_invalid"))
    end
    ...
    targetPlayer.setOrga(args.orga, args.grade)
    Core.SavePlayer(targetPlayer, function() end)

    args.playerId.setOrga(args.orga, args.grade)
    if Config.AdminLogging then
      ESX.DiscordLogFields("UserActions", "Set Organisation /setorga Triggered!", "pink", {
        { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
        { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
        { name = "Target", value = args.playerId.name, inline = true },
        { name = "Orga", value = args.orga, inline = true },
        { name = "Grade", value = args.grade, inline = true },
      })
    end
  end,
  true,
  {
    help = TranslateCap("command_setorga"),
    validate = true,
    arguments = {
      { name = "playerId", help = TranslateCap("commandgeneric_playerid"), type = "player" },
      { name = "orga", help = TranslateCap("command_setorga_orga"), type = "string" },
      { name = "grade", help = TranslateCap("command_setorga_grade"), type = "number" },
    },
  }
)
...
ESX.RegisterCommand(
  ...
  function(xPlayer, args, showError)
    ...
    targetPlayer.setGang(args.gang, args.grade)
    Core.SavePlayer(targetPlayer, function() end)
    ...
    if Config.AdminLogging then
      ESX.DiscordLogFields("UserActions", "Set Gang /setgang Triggered!", "pink", {
        ...
        { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
        { name = "Target", value = args.playerId.name, inline = true },
        { name = "Gang", value = args.gang, inline = true },
        { name = "Grade", value = args.grade, inline = true },
      })
    end
  end,
...
ESX.RegisterCommand("job2", { "user", "admin" }, function(xPlayer, _, _)
  ...
  print(("%s, your job 2 is: ^5%s^0 - ^5%s^0"):format(xPlayer.getName(), job2.name or "nil", job2.grade_label or "nil"))
end, false)

ESX.RegisterCommand("orga", { "user", "admin" }, function(xPlayer, _, _)
  local orga = xPlayer.getOrga()
  print(("%s, your orga is: ^5%s^0 - ^5%s^0"):format(xPlayer.getName(), orga.name or "nil", orga.grade_label or "nil"))
end, false)

ESX.RegisterCommand("gang", { "user", "admin" }, function(xPlayer, _, _)
  local gang = xPlayer.getGang()
  print(("%s, your gang is: ^5%s^0 - ^5%s^0"):format(xPlayer.getName(), gang.name or "nil", gang.grade_label or "nil"))
end, false)
